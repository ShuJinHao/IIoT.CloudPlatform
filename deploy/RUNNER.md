...
本文只描述 runner 准备要求。创建 OS 用户、授权 Docker 和写入 GitHub 注册 token 是显式基础设施维护；标准 Runner 安装/升级由工作区 `Deploy.ps1 -InstallRunner` 执行，不属于普通应用部署。三项目上传部署统一入口见 [上传部署总览](../../docs/上传部署总览.md)。

## 目标

CloudPlatform 标准生产发布只由工作区根 `Deploy-Changed.ps1` 创建短时、HMAC、单次内部 dispatch receipt，再调度本机构建、Harbor push 和 SSH 事务。self-hosted workflow 只允许白名单生产状态 inspect 或用户明确授权的 emergency 操作；`cloud-routine-request.yml` 不能接收部署请求。
...
- 只读 inspect workflow 不 checkout 或构建源码，也不读取应用发布凭据。
...
只读状态 inspect workflow 只需要生产目录定位：

```text
DEPLOY_TARGET_DIR=/data/iiot-platform/cloud/deploy
```

标准发布所需 Harbor、Cloud、管理员和数据库凭据来自 macOS Keychain，不从 GitHub Secrets 或 Markdown fallback。Emergency workflow 如确需独立凭据，必须由用户明确授权、受 production environment 保护并与普通部署完全分离；不得把 emergency secret 列表复制成标准部署准备清单。
