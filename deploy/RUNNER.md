...
长期方向是取消长期生产 secret 常驻 self-hosted runner，改为 GitHub OIDC + Vault 或等价短期凭据签发。短期内必须至少使用 GitHub production environment protection、专用非 root runner 用户、最小 Harbor 权限、`DEPLOY_ENV_FILE` 0600 写入和 workflow 灾备确认词；不得把 runner 当成日常生产发布入口。
