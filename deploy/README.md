...
> 读取边界：日常部署只读本页“日常标准入口”和当前命中的服务/故障章节；历史状态、旧事务和未命中的维护章节不默认读取。
...
pwsh ./deploy/Deploy-FromZero.ps1 -Targets Cloud,AICopilot,Edge -ConfirmFromZero
```
...
- 日常部署必须由 `Deploy-Changed.ps1` 确认 clean/main 并 push 已有 HEAD；未提交内容直接阻断，执行前后 tracked 文件必须零变化。
- 从零 Cloud 阶段只执行 release history 导出/恢复、基础设施、migration、seed 和健康；本机密钥只从 macOS Keychain canonical schema 读取。缺键时远端零写入；不得创建设备、注册 `ClientCode` 或轮换设备 bootstrap secret。
...
- 显式 `workflow_dispatch`：只由 GitHub hosted `windows-latest` 构建 runtime、installer artifact 和 Velopack 离线 artifacts，上传 Actions artifact 后停止；不连接 Cloud、不写 `${EDGE_UPDATES_DIR}`、不构成生产发布证据。
...
- Edge 生产更新内容必须由根入口显式传 `-ReleaseNotes` 或 `-ReleaseNotesPath`；离线 `workflow_dispatch` 的 `release_notes` 只进入离线 artifact。Cloud Human 发布接口和 host/plugin release upsert 会拒绝 `Published` 空更新内容。

GitHub 完整打包流程固定为：

```text
IIoT.EdgeClient workflow_dispatch
-> windows-latest 构建 edge-installer-artifact 和 edge-velopack-releases
-> 上传 GitHub Actions artifacts
-> 停止，不连接 self-hosted runner 或 Cloud 发布 API
```
...
`iiot-httpapi` 以可写方式挂载 `${EDGE_UPDATES_DIR}` 到 `/app/edge-updates`，仅用于内网受控 Edge HTTP 发布；`nginx-gateway` 仍以只读方式挂载同一目录。HttpApi 通过 `EdgeInstallerArtifacts__RootPath=/app/edge-updates/installers` 读取安装素材，并通过 `EdgeInstallerArtifacts__VelopackReleasesBaseUrl=${PUBLIC_BASE_URL}/edge-updates/velopack` 返回运行时更新源。HTTP 宿主上传成功后，服务端会校验 `installer-artifact.json` v2；物理三仓下 Host manifest 允许合法 `modules: []`，独立插件 zip 必须分别通过 `plugin-packages` 发布。公开下载目录、Edge catalog、Human catalog 和首装链路只按这些 Cloud release 记录决定版本集合，文件系统只验证已登记产物。下载验收分别验证 Host installer/Setup/Velopack 与指定插件 zip 的 catalog、大小、SHA256 和 HTTP 字节，不能再要求插件出现在 Host manifest 中。已登记同版本插件不会被宿主发布覆盖。文件存在性以落盘包为准，可见性和生命周期以 DB 状态为准。保留策略默认每个 stable/runtime/component 按 SemVer 最多保留最新 3 次，旧版本先 Archived/Deprecated，只有已归档且无设备在用的文件才会回收。登录 Cloud 后，“客户端下载中心 -> 首装下载”会按已登记版本对应的 `installer-artifact.json` v2 选择一份 `host/` 和所选 `plugins/<ModuleId>/`，只向本次下载的安装器 payload 注入 `launcher/iiot-binding.json`、`launcher/iiot-enabled-plugins.json` 和 `launcher/launcher.update.json`，并返回真正的 `.exe`。其中 `launcher/iiot-binding.json` 是唯一允许携带启动密钥的输入；选择清单和更新配置不得携带密钥。组包链在完成前必须从基础包和所选插件中彻底剔除任何 `iiot-plugin-binding.json`，最终 payload 中该文件数量必须为零。这些待应用配置不写回素材目录、不落盘到共享模板、不写日志。
...
管理员工号和密码的机器真值分别是 macOS Keychain canonical 项 `cloud.seed-admin-no` 与 `cloud.seed-admin-password`。标准从零部署只从 Keychain 读取并在内存中生成远端 `.env`，不从 GitHub secret、Markdown 或旧 env 回退，也不把值放入仓库、文档或日志。普通增量部署不读取、随机化、覆盖或猜测管理员密码。

`iiot-migration` 的播种规则是：

- 数据库中不存在任何 `Admin` 用户时，使用从 canonical Keychain 写入部署环境的 `SEED_ADMIN_NO` 和 `SEED_ADMIN_PASSWORD` 创建首个管理员。
...
1. 候选仓库必须是 clean、已提交的 `main`；根入口可以 push 现有 HEAD，但不会创建提交或修改任何 tracked 文件。
2. push/PR 的默认 CI 只执行 Architecture、Security 与受影响 Business，并按当前 discovery 对账。全量、coverage、mutation、duplication、Quality、浏览器全量和 CrossProject 只在用户显式选择对应模式时运行，不存在 nightly 自动全量。
3. 在工作区根运行 `pwsh ./deploy/Deploy-Changed.ps1 -Targets Cloud`。入口读取生产基线，按 Git diff、项目引用和服务依赖计算受影响闭包；无法归属时停止，不退化 `-All`。
4. 完全相同 SHA 和 changed-files scope 的绿色证据直接复用；只补缺失的受影响 Architecture、Security、DeploymentContract。部署不运行普通业务测试，也不借失败修改源码、测试、规则或文档。
5. 内部 `Deploy.ps1 -Target Cloud -Services ...` 只构建、推送并发布计划中的不可变镜像；迁移按服务闭包显式加入。日常应用发布不安装或覆盖服务器 support files。
6. 迁移、配置检查、健康检查、Edge 下载链、镜像 digest 和回滚登记全部通过后才报告成功；入口结束时再次核对候选 HEAD 和 tracked 文件零变化。失败时停止并保留本 invocation 证据。

Compose、nginx、Runner 或服务器 support files 的安装/升级属于单独的基础设施维护，不是普通应用发布步骤。只有明确进入该维护模式时，才按后续 support transaction 章节使用 `Invoke-WorkspaceDeploy.ps1`、`local-release.sh` 及相关恢复契约。

下列 GitHub secrets 仅用于已明确授权的 Runner/基础设施维护链，不是本机标准部署的密钥来源；本机标准流程只从工作区 canonical Keychain schema 取值：
