...
Analyzer 使用 Roslyn `IOperation` 和同编译单元方法调用图追踪，必须识别字段、属性、局部变量和参数中存储后再调用的委托，也必须识别由 `IAiReadRequest<>` 约束确定的泛型 handler。AiRead 授权审计属于独立 pipeline behavior，不得作为 handler/helper 写入例外。

跨项目调用图必须 fail-closed：

- `IReadOnlyQueryPort` 是编译型直接信任标记，只能由纯读端口直接继承；同时暴露 Add/Update/Delete/Remove/Save/Execute 类写方法的混合端口不得获得读信任，写仓储也不得通过间接继承 `IReadRepository<>` 获得信任。
- AiRead 调用未分类的外部 `IIoT.*` 接口必须报 `CLOUDARCH004`；不得因实现在另一 csproj 而结束调用图。
- Analyzer 还必须把直接标记端口的真实实现方法当作新的根扫描；该实现到达 Dapper `Execute*`、EF 写 API、仓储写入或命令发送时仍必须失败。
...
`IIoT.CloudPlatform.AnalyzerTests` 是独立 required 测试项目，当前 61 case，覆盖稳定 ID、默认 Error、正例、反例、alias、generic 及 `IAiReadRequest<>` 约束、helper、接口分派、跨文件、inheritance、callback/lambda/字段、属性、局部和参数存储委托、外部类型的 callback 接收者、`DynamicInvoke`、strict raw SQL（含 quoted identifier/string/comment 边界）、未分类生产层 fail-closed、dynamic fail-closed 和 no-false-positive。Analyzer 必须对生成代码继续执行分析并上报，不能用 `<auto-generated/>` 或 `.g.cs` 逃逸。`scripts/tests/TestCloudArchitectureAnalyzerFixtures.sh` 另外创建临时真实 csproj，要求 valid 6 build 成功、invalid 15（含生成代码负例和真实跨 csproj AiRead 端口→Dapper 写实现）每个只以预期 `CLOUDARCH*` 稳定失败，并以 3 个 suppression bypass 证明诊断不能关闭。这两层都是 `cloud-ci / build-test` required 门禁，不可 `continue-on-error`。
