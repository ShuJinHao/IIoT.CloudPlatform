...
`IIoT.CloudPlatform.AnalyzerTests` 是独立 required 测试项目，当前 72 case，覆盖稳定 ID、默认 Error、正反例、generic/委托/隐式调用/生命周期、递归 writable capability、跨程序集 effect summary、开放分派、static field initializer、strict raw SQL、未分类生产层与 no-false-positive。Analyzer 必须对生成代码继续执行分析并上报，不能用 `<auto-generated/>` 或 `.g.cs` 逃逸。`scripts/tests/TestCloudArchitectureAnalyzerFixtures.sh` 创建临时真实 csproj，必须精确输出 `valid=8 / invalid=15 / callGraphBypass=20 / suppressionBypass=3 / contractsMetadataIdentities=9`；其中安全缓存正反例必须以 `ProjectReference` 引用真实 `IIoT.Services.Contracts.csproj`，binding probe 必须从真实 build output 证明 Analyzer 使用的 9 个 `IIoT.Services.Contracts` metadata identity 各恰好解析一次，任一缺失、改名或 Analyzer 新增未登记 identity 都必须 fail-closed。旧 schema、来源不匹配、重复伪造 manifest、非受管预编译引用和损坏 digest 都只能以 `CLOUDARCH004` 失败。这两层都是 `cloud-ci / build-test` required 门禁，不可 `continue-on-error`。
