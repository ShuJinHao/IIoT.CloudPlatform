...
jobs:
  build-test:
    ...
    steps:
      ...
      - name: Existing Phase 0 gate
        shell: pwsh
        run: Write-Host phase-zero
...
'@
    ...
    Write-Utf8File -Path (Join-Path $root '.github/workflows/cloud-image.yml') -Content "name: old-image`n"
...
try {
    Assert-Pass -Name 'reference schema locks reviewed receipt constants' -Action {
        ...
        if ($schema.additionalProperties -ne $false -or
            ...
            [string]$schema.properties.ruleId.const -cne 'CLOUD-BASELINE-MIG-001' -or
            [string]$schema.properties.approvedBy.const -cne 'ShuJinHao' -or
            [long]$schema.properties.changes.maxItems -ne 5000 -or
            @($schema.properties.owner.enum).Count -ne 6 -or
            @($schema.properties.owner.enum) -cnotcontains 'Cloud.Infrastructure' -or
            @($schema.properties.ruleIds.items.enum).Count -ne 9 -or
            @($schema.properties.ruleIds.items.enum) -cnotcontains 'CLOUD-CACHE-001' -or
            @($schema.properties.ruleIds.items.enum) -cnotcontains 'CLOUD-TEST-GOV-001B' -or
            @($schema.properties.ruleIds.items.enum) -cnotcontains 'CLOUD-BASELINE-TRUST-UPGRADE-001' -or
            @($schema.'$defs'.counts.required).Count -ne 17 -or
    ...
    $duplicateMigrationSelfTest = New-AuthorizationFixture -MutateTemplate {
        param($root)
        $path = Join-Path $root '.github/workflows/cloud-ci.yml'
        [IO.File]::AppendAllText(
            $path,
    ...
    Assert-Rejected -Name 'required workflow cannot duplicate migration self-test reference' -ExpectedCode 'TRUST' -Action {
        Invoke-Validation `
            -Fixture $duplicateMigrationSelfTest `
            -Base $duplicateMigrationSelfTest.Authorization `
            -Candidate $duplicateMigrationSelfTest.Candidate
    }

    $renamedRequiredJob = New-AuthorizationFixture -MutateTemplate {
        param($root)
        $path = Join-Path $root '.github/workflows/cloud-ci.yml'
        $text = [IO.File]::ReadAllText($path).Replace(
            "  build-test:`n",
            "  renamed-build-test:`n")
        Write-Utf8File -Path $path -Content $text
    }
    $renamedRequiredJob = Complete-Candidate -Fixture $renamedRequiredJob
    Assert-Rejected -Name 'single required workflow job name is pinned' -ExpectedCode 'TRUST' -Action {
        Invoke-Validation `
            -Fixture $renamedRequiredJob `
            -Base $renamedRequiredJob.Authorization `
            -Candidate $renamedRequiredJob.Candidate
    }
    ...
    Assert-Rejected -Name 'ordinary receipt cannot replace trust implementation' -ExpectedCode 'TRUST' -Action {
        Invoke-DescribeResult -Fixture $ordinaryTrustChange -RuleIdsCsv 'CLOUD-ARCH-001'
    }
...
if ($script:Failed -ne 0) {
    throw "Cloud baseline migration validator self-tests failed: passed=$($script:Passed) failed=$($script:Failed)."
}

Write-Host "Cloud baseline migration validator self-tests passed: $($script:Passed)/$($script:Passed)."
