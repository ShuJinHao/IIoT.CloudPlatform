...
### 13.2 2026-07-17 审计后重新关单矩阵

本矩阵是当前终态，替代第 13.1 节的历史登记，但不删除历史快照。唯一机器入口为 `scripts/testing/baselines/three-repository-closure-predicates.json`；机器结果为 `blocking=14`、`passed=14`、`failed=0`、`notRun=0`。独立严格复审见[审计后重新关单严格复审](../artifacts/audits/2026-07-17/三项目测试架构治理-审计后重新关单严格复审-20260717.md)。

| # | AND 完成条件 | 当前不可变证据摘要 | 结论 |
|---:|---|---|---|
| 1 | 错误治理资产物理退役 | 三仓 frozen tracked-file denylist 零命中；只允许正式规则、负例和历史追溯文本 | `PASS` |
| 2 | Edge 模切与本地产物归零 | 活动 unexpected=0；治理 allowlist 精确两文件；retired declaration=41；行为负例 8；可再生目录=0 | `PASS` |
| 3 | Edge 中性插件测试 seam | 中性 TestPlugin 生命周期、完成、取消契约进入 Edge required run | `PASS` |
| 4 | 三仓测试资产唯一分类 | Edge 32/1332；Cloud 18/725、required 661；AI 25/1026、required 1012、manual 14 | `PASS` |
| 5 | 旧大桶/filter/helper 物理清理 | 冻结 denylist、重复 helper 定义、inventory 行为负例和仓内 duplication ratchet 闭合 | `PASS` |
| 6 | Required 对账全绿 | Edge 1332/1332；Cloud 661/661；AI 1012/1012；三仓均 failed=0、skipped=0、attempt=1 | `PASS` |
| 7 | Analyzer 与违规 fixture | Cloud 真实 Contracts identities=9、valid=8、invalid=15、callGraph=20、suppression=3；Edge/AI 正反 gate 同步通过 | `PASS` |
| 8 | 高风险语义门禁 | 10 个冻结 identity 均绑定 test、runner、result、run 和最终 HEAD | `PASS` |
| 9 | 测试归真实失效层 | TestKind/Runtime/Capability/ProjectReference 闭合矩阵及非法依赖图负例均生效 | `PASS` |
| 10 | duplication/coverage/mutation/compatibility 稳定 | 三仓 baseline/阈值无放宽；Cloud compatibility 绑定最终 Edge clean source-state；AI mutation 58/58 | `PASS` |
| 11 | 并行与 25 分钟目标 | final run 样本 Edge 1284s、Cloud 788s、AI 1372s，nearest-rank p95=1372s≤1500s；required 数不减少 | `PASS` |
| 12 | digest 与非生产联合验收 | contract digest、六组 duplication 扫描、Release authoritative 联合验收绑定同一组三仓 clean HEAD/tree | `PASS` |
| 13 | 正式规则沉淀 | 唯一 14 项 catalog 将每项映射到正式规则与自动 gate；复盘/历史不是唯一来源 | `PASS` |
| 14 | 外部/生产边界真实记录 | 三候选 PR 与 evidence PR 均 Draft/Open/未 merge；未部署、未调用生产 CD/Schema/Data/服务器/现场/LiveExternal | `PASS` |
...
2026-07-16 曾登记 **14/14、100%**；2026-07-17 客观审核将计划重新打开，审计后修复随后形成了新的三仓 clean candidate、attempt 1 required CI、跨仓联合证据、14 项机器 catalog、远端 evidence commit A 和独立严格复审。当前机器结果为 `PASS=14`、`FAIL=0`、`NOT-RUN=0`，第 15.2 节所有 blocking 批次均为 `PASS`。

因此当前唯一正式状态重新登记为 **14/14、100%、已关单**。关单依据是第 13.2 节当前矩阵、[严格复审报告](../artifacts/audits/2026-07-17/三项目测试架构治理-审计后重新关单严格复审-20260717.md)和 evidence-only Draft PR #37 的普通追加 commit 链。commit A 冻结 reopened 输入，commit B 冻结首次严格复审；复审后发现的 command 可执行性和 AI coverage 直接绑定缺口必须由后续普通 commit 加固，禁止 amend/force push。最终 full SHA/tree/manifest 只在严格复审和最终交付登记，不反向写回本计划以避免自引用。
...
| ID | 仓/层级 | 必须闭合的客观结果 | 最终状态 |
|---|---|---|---|
| `AI-RAG-E2E-EXEC-001` | AI | RAG 上传→索引→搜索→MCP 测试被 xUnit 实际发现和执行，并进入 inventory、迁移账本与 required CI | `PASS` |
| `AI-COMPAT-SURFACE-RATCHET-001` | AI | encv1 所有公开 legacy surface 均有逐符号 caller ratchet；新增调用、漏登记 surface 和放宽阈值负例均失败 | `PASS` |
| `CLOUD-ANALYZER-CONTRACT-BINDING-001` | Cloud | Analyzer 对真实 `Services.Contracts` metadata 成功绑定；缺失/改名时 fail-closed，正反 fixture 进入 required CI | `PASS` |
| `CLOUD-COMPAT-SOURCE-BINDING-001` | Cloud/Edge | 外部 HEAD 作为证据身份记录，兼容性阻断以 clean consumer source-state digest、逐文件 SHA、pattern 和 call count 为语义判据 | `PASS` |
| `EDGE-STARTUP-EXCEPTION-ALLOWLIST-001` | Edge | 启动适配器只降级显式可恢复异常；未知异常保持同一实例抛出；不伪造取消语义 | `PASS` |
| `EDGE-DIECUT-ACTIVE-INPUT-ALLOWLIST-001` | Edge | 活动源码/配置/solution/打包输入模切零命中；41 条退役账本仅作为精确 allowlist 的迁移证据保留 | `PASS` |
| `GOV-CLOSE-PREDICATE-CATALOG-001` | 工作区 | 14 项 AND 条件均有封闭输入集、allowlist/denylist、机器命令、阈值、HEAD/run/evidence 绑定和原因码 | `PASS` |
| `GOV-CLOSE-EVIDENCE-ANCHOR-001` | 工作区/GitHub | 最终根证据、catalog、三仓 HEAD/run/artifact digest 被非生产 Git/GitHub 不可变对象锚定 | `PASS` |
| `FINAL-ACCEPTANCE-REOPEN-001` | 三仓联合 | 三仓 clean candidate 的 required、coverage、mutation、duplication、compatibility、contract digest、非生产联合验收和严格 14 项复审全部 PASS | `PASS` |
...
完成前禁止写“基本完成”“条件通过”“主体 100%”或折算百分比。当前已按公式完成并更新第 13.2 节，同时保留第 13.1 节 2026-07-16 历史快照供审计。

### 15.4 关单证据锚点

- evidence-only Draft PR：[CloudPlatform #37](https://github.com/ShuJinHao/IIoT.CloudPlatform/pull/37)，保持 Draft/Open，禁止 merge、tag、deploy 或生产操作。
- commit A：`ba54735afedee843064dce739a3e1e1acd9c85a6`；tree：`53f04364ca2994fac0163caebedae0e1d81d4a19`；manifest SHA-256：`761fb7b494d7bdc67a17ab126cb44311cb43b78f511811d501993c37297eaa1f`。
- 机器 catalog SHA-256：`a9f340d464389df5c2f1d36514949feefa0a30c6131d7e87d3bb06d2b4b35805`；机器 result SHA-256：`edb0f44f4d502d49ec9847dc4a3b91b9694aaa4bc49a9c074328cff1d1598047`。
- 严格复审：[三项目测试架构治理-审计后重新关单严格复审-20260717.md](../artifacts/audits/2026-07-17/三项目测试架构治理-审计后重新关单严格复审-20260717.md)，14 项 blocking 全部 `PASS`。
- commit B 冻结首次已关单快照、严格复审和 commit A 元数据；其后的普通加固 commit 只更新 catalog/result、闭合快照、严格复审和 manifest。最终 full SHA/tree/manifest 在 evidence PR 推送后登记到根复审报告与最终交付，不写回本计划，避免无限自引用。
