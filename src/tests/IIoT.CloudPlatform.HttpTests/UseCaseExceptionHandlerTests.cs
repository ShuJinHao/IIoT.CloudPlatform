...
public sealed class UseCaseExceptionHandlerTests
{
        ...
        string expectedDetail)
    {
        ...
        if (exceptionKind == "unknown")
        {
            ...
            await AssertCallerCancellationIsUnhandledAsync(handler);
        }
    }

    private static async Task AssertCallerCancellationIsUnhandledAsync(UseCaseExceptionHandler handler)
    {
        ...
        var handled = await handler.TryHandleAsync(context, expected, cancellation.Token);
