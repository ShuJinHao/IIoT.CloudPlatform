...
{
    [Fact]
    public void RouteSurfaceApiExplorerConvention_ShouldClassifyExactControllerPrefixes()
    {
        Assert.Equal(
            "bootstrap",
            RouteSurfaceApiExplorerConvention.ResolveGroupName(
        ...
        Assert.Equal(
            "edge",
            RouteSurfaceApiExplorerConvention.ResolveGroupName(
                "api/v1/edge/capacity",
                "EdgeCapacity",
                "ReceiveHourly"));
        Assert.Equal(
            "machine",
            RouteSurfaceApiExplorerConvention.ResolveGroupName(
                "api/v1/machine/edge-release",
                "MachineEdgeRelease",
                "GetRelease"));
        Assert.Equal(
            "human",
            RouteSurfaceApiExplorerConvention.ResolveGroupName(
                "api/v1/human/devices",
                "HumanDevice",
                "GetAll"));
    }
