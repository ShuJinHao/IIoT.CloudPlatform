...
public sealed partial class CloudProductionFlowTests : IAsyncLifetime
{
    ...
    private readonly IIoTAppFixture _fixture;
    private readonly CloudTestDriver _driver;

    public CloudProductionFlowTests()
    {
        _fixture = new IIoTAppFixture();
    ...
    private Task AuthenticateAsAdminAsync() => _driver.AuthenticateAsAdminAsync();

    private Task AuthenticateAsEdgeAsync(Guid deviceId) => _driver.AuthenticateAsEdgeAsync(deviceId);

    private static Task<string> ReadJwtTokenAsync(HttpResponseMessage response) =>
        CloudTestDriver.ReadJwtTokenAsync(response);
    ...
    private Task<TestDeviceRegistration> CreateTestDeviceRegistrationAsync(string prefix) =>
        _driver.CreateTestDeviceRegistrationAsync(prefix);
    ...
    {
        return CloudTestDriver.CreateBootstrapRequest(device, bootstrapSecret, path);
    }
    ...
    private static HttpRequestMessage CreateBootstrapRequest(
        ...
        string path = "/api/v1/bootstrap/device-instance")
    {
        return CloudTestDriver.CreateBootstrapRequest(clientCode, bootstrapSecret, path);
    }
    ...
    private Task PostJsonAsync(string path, object request) => _driver.PostJsonAsync(path, request);

    private Task<T> PostJsonAsync<T>(string path, object request) => _driver.PostJsonAsync<T>(path, request);
