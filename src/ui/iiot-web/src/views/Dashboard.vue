  ...
  <div class="space-y-7">
    <section class="flex items-start justify-between gap-6">
      <div>
        ...
        <h2 class="text-[var(--fs-4xl)] font-[var(--fw-strong)] leading-tight tracking-[0] text-[var(--text-0)] dark:text-[var(--text-0)]">{{ t('dashboard.title') }}</h2>
    ...
    <section class="grid grid-cols-[minmax(0,1fr)_260px] gap-7 max-[1180px]:grid-cols-1">
      <div class="min-w-0 space-y-7">
        <div class="grid grid-cols-3 gap-6 max-[1060px]:grid-cols-2 max-[820px]:grid-cols-1">
          <article
            ...
            :key="card.label"
            class="min-h-[132px] rounded-[var(--radius-lg)] p-5 text-[var(--text-0)]"
            :style="{ background: card.background }"
          >
            <component :is="card.icon" class="mb-8" :size="17" :stroke-width="2.4" />
            <div class="text-[27px] font-[var(--fw-strong)] leading-none tracking-[0] tabular-nums">{{ card.value }}</div>
            <div class="mt-2 text-[var(--fs-sm)] font-[var(--fw-bold)] text-[var(--text-1)]">{{ card.label }}</div>
            <div class="mt-1 text-[var(--fs-xs)] font-[var(--fw-semibold)] text-[var(--text-2)]">{{ card.helper }}</div>
          </article>
        </div>
        ...
        <div class="grid grid-cols-[minmax(0,1.45fr)_minmax(280px,0.85fr)] gap-7 max-[1180px]:grid-cols-1">
          <section class="rounded-[var(--radius-xl)] bg-white p-6 shadow-[var(--shadow-sm)] ">
            <div class="mb-8 flex items-start justify-between gap-4">
              <div>
                <h3 class="text-[var(--fs-2xl)] font-[var(--fw-strong)] text-[var(--text-0)] dark:text-[var(--text-0)]">{{ t('dashboard.productionTrend') }}</h3>
          ...
          <section class="rounded-[var(--radius-xl)] bg-white p-6 shadow-[var(--shadow-sm)] ">
            <div class="mb-7 flex items-center justify-between">
              <div>
                <h3 class="text-[var(--fs-2xl)] font-[var(--fw-strong)] text-[var(--text-0)] dark:text-[var(--text-0)]">{{ t('dashboard.analysis') }}</h3>
                <p class="mt-1 text-[var(--fs-base)] font-[var(--fw-semibold)] text-[var(--muted-foreground)]">{{ t('dashboard.analysisSub') }}</p>
              </div>
            </div>
            ...
            <div class="space-y-3">
              <router-link
                ...
                :key="item.label"
                :to="item.to"
                class="flex h-14 items-center justify-between rounded-[var(--radius-md)] bg-[var(--bg-2)] px-4 text-[var(--fs-base)] font-[var(--fw-strong)] text-[var(--text-0)] transition-colors hover:bg-[var(--bg-3)] dark:bg-[var(--bg-2)] dark:text-[var(--text-0)]"
              >
        ...
        <section class="rounded-[var(--radius-xl)] bg-white p-6 shadow-[var(--shadow-sm)] ">
          <div class="mb-5 flex items-center justify-between">
            <h3 class="text-[var(--fs-2xl)] font-[var(--fw-strong)] text-[var(--text-0)] dark:text-[var(--text-0)]">{{ t('dashboard.recentAlerts') }}</h3>
          ...
          <div class="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border)]">
            <div class="grid grid-cols-[120px_minmax(0,1fr)_140px_92px] bg-[var(--bg-2)] px-4 py-3 text-[var(--fs-sm)] font-[var(--fw-bold)] text-[var(--muted-foreground)] dark:bg-[var(--bg-2)]">
            ...
            <div v-if="events.length" class="divide-y divide-[var(--border)]">
              <div
                ...
                class="grid grid-cols-[120px_minmax(0,1fr)_140px_92px] items-center px-4 py-4 text-[var(--fs-base)] font-[var(--fw-semibold)] text-[var(--text-0)] dark:text-[var(--text-0)]"
              >
      ...
      <aside class="space-y-6">
        ...
        <section class="rounded-[22px] bg-white p-5 shadow-[var(--shadow-sm)] ">
          <h3 class="mb-5 text-[var(--fs-xl)] font-[var(--fw-strong)] text-[var(--text-0)] dark:text-[var(--text-0)]">{{ t('dashboard.deviceStatus') }}</h3>
          <div class="space-y-3">
            <div v-for="row in statusRows" :key="row.label" class="flex items-center justify-between rounded-[var(--radius-md)] bg-[var(--bg-2)] px-3 py-3 dark:bg-[var(--bg-2)]">
              <span class="flex items-center gap-3 text-[var(--fs-base)] font-[var(--fw-bold)] text-[var(--text-0)] dark:text-[var(--text-0)]">
                <i class="size-2.5 rounded-full" :style="{ background: row.color }"></i>
                {{ row.label }}
              </span>
              <span class="font-mono text-[var(--fs-base)] font-[var(--fw-strong)] text-[var(--text-0)] dark:text-[var(--text-0)]">{{ row.value }}</span>
            </div>
          </div>
        </section>

        <section class="rounded-[22px] bg-white p-5 shadow-[var(--shadow-sm)] ">
          <h3 class="mb-5 text-[var(--fs-xl)] font-[var(--fw-strong)] text-[var(--text-0)] dark:text-[var(--text-0)]">{{ t('dashboard.shiftDuty') }}</h3>
          <div class="space-y-2">
            <div v-for="member in teamMembers" :key="member.name" class="flex items-center justify-between rounded-[var(--radius-md)] bg-[var(--bg-2)] px-3 py-3 dark:bg-[var(--bg-2)]">
              <span class="flex min-w-0 items-center gap-3">
                ...
                <span class="min-w-0">
                  <strong class="block truncate text-[var(--fs-sm)] font-[var(--fw-strong)] text-[var(--text-0)] dark:text-[var(--text-0)]">{{ member.name }}</strong>
