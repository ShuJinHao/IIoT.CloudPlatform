...
function getFieldDefinition(definition: PassStationTypeDefinitionDto, key: string) {
  return definition.fields.find((field) => field.key === key) ?? null;
}

function getFieldLabel(definition: PassStationTypeDefinitionDto, key: string) {
  return commonFieldLabels[key] ?? getFieldDefinition(definition, key)?.label ?? key;
}

function getListClassName(field: PassStationFieldDefinitionDto | null, key: string) {
  if (key === 'completedTime' || key === 'receivedAt') return 'time-cell';
  if (field?.type === 'number' || field?.type === 'integer') return 'mono';
  return undefined;
}

function getDetailClassName(field: PassStationFieldDefinitionDto | null, key: string) {
  if (commonFieldClassNames[key]) return commonFieldClassNames[key];
  if (field?.type === 'number' || field?.type === 'integer') return 'mono-val';
  return undefined;
}

function getRecordValue(record: PassStationListItemDto, key: string) {
...
function getDetailValue(detail: PassStationDetailDto, key: string) {
...
function buildColumn(definition: PassStationTypeDefinitionDto, key: string): PassStationColumnSchema {
...
function buildDetailField(definition: PassStationTypeDefinitionDto, key: string): PassStationDetailFieldSchema {
  const field = getFieldDefinition(definition, key);
  return {
    key,
    label: getFieldLabel(definition, key),
...
export function buildPassStationSchema(definition: PassStationTypeDefinitionDto): PassStationSchema {
...
export function buildPassStationSchemaMap(definitions: PassStationTypeDefinitionDto[]) {
...
export function getPassStationSchema(schemas: Record<string, PassStationSchema>, typeKey: string) {
