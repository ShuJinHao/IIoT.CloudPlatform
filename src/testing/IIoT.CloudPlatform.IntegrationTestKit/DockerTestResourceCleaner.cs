...
internal static class DockerTestResourceCleaner
{
    ...
    private static async Task CleanupNamedVolumeAsync(string volumeName)
    {
        ...
        Console.Error.WriteLine(
            $"[IIoT.CloudPlatform.IntegrationTestKit] Failed to remove test volume '{volumeName}' after {MaxCleanupAttempts} attempts.");
    }
    ...
    private static async Task RemoveContainersUsingVolumeAsync(string volumeName)
    {
        ...
        foreach (var containerId in containerIds)
        {
            ...
            {
                Console.Error.WriteLine(
                    $"[IIoT.CloudPlatform.IntegrationTestKit] Failed to remove test container '{containerId}' while cleaning volume '{volumeName}'.");
            }
        }
    }
